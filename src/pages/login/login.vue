...
const onGetphonenumberSimple = async () => {
  await checkedAggreePrivacy()
  const res = await postLoginWxMinSimpleAPI('17320507697')
  loginSuccess(res.result)
}
...
const loginSuccess = (profile: LoginResult) => {
  ...
  memeberStore.setProfile(profile)
  // 成功提示
...
// #ifdef H5
// 传统表单登录， 测试账号13123456789 密码：123456，测试账号仅开发学习使用。
const form = ref({
  account: '13123456789',
  password: '',
})
...
const checkedAggreePrivacy = async () => {
...
<template>
  <view class="viewport">
    ...
    <view class="login">
      <!-- 网页端表单登录 -->
      <!-- #ifdef H5 -->
      <input type="text" v-model="form.account" class="input" placeholder="请输入用户名/手机号码" />
      <input type="text" v-model="form.password" password class="input" placeholder="请输入密码" />
      ...
      <view class="button-privacy-wrap">
        <button
          ...
          @tap="checkedAggreePrivacy"
        >
          请先阅读并勾选协议
        </button>
        <button class="button phone" open-type="getPhoneNumber" @getphonenumber="onGetphonenumber">
          <text class="icon icon-phone">手机快捷登录</text>
        </button>
      </view>
      <!-- #endif -->
      <view class="extra">
      ...
      <view class="tips" :class="{ animate_shakeY: isAgreePrivacyShakeY }">
        <label @tap="isAgreePrivacy = !isAgreePrivacy" class="label">
          <radio class="radio" color="#28bb9c" :checked="isAgreePrivacy"></radio>
          <text>登录/注册即视为你同意协议</text>
        </label>
